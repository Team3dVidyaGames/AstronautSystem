...
event DarkMatterCollected(address sender, string name, uint256 amount, uint256 RestTime);
...
constructor(uint256 _DmPerBlock, address _vault, address _inventory, IDarkMatter _dm, IERC20 _vidya, uint256 _astronautID, uint256 _O2tanksID, uint256 _DMID){
...
function register(uint256 _tokenID, string _name) nonReentrant{
    ...
    require(!astronaut.burned && (astronaut.templateID == astronautID), "Space Station: Astronaut Does Not Exisit.");
    require(inventory.balanceOf(msg.sender, _tokenID) > 0, "Space Station: Mission Controls not granted");
    ...
    data.rate = (data.feature[0] + data.feature[1] + 1) * (50 + data.feature[3]);
...
function collectDarkMatter(uint256 _astronautID, uint256 numberOfTanks){
    ...
    require(inventory.balanceOf(msg.sender, _astronautID) > 0, "Space Station: Permission not granted");
    require(inventory.balanceOf(msg.sender, O2tanksID)>= numberOfTanks && numberOfTanks > 0, "Space Station: Miscalculation on O2 tanks");
    collectionData storage data = astronauts[_astronautID];
    require(data.collectionStart <= block.timestamp, "Space Station: Astronaut not ready for Mission");
    ...
    data.collectionStart = block.timestamp + ((256 - data.feature[2]) * timeDiffernce) / (numberOfTanks * 512);
    data.experience += dmToMint;
    DarkMatter.mint(msg.sender, dmToMint);

    emit DarkMatterCollected(msg.sender, data.name, dmToMint, data.collectionStart);

}



function trainAstronaut(uint256 _tokenID, uint256 _numDMNFT, uint256 _O2tanks, uint256 _vidya, uint256 _dm) nonReentrant{
    ...
    if(_vidya > 0 && data.feature[0] < 256){
        require(Vidya.balanceOf(msg.sender)>=_vidya, "Space Station: Misscalculation on Vidya amount.");
        Vidya.safeTrasferFrom(msg.sender, vault, _amount);
        xpGained[0] = _vidya;
    }

    if(_dm > 0 && data.feature[1] < 256){
        require(DarkMatter.balanceOf(msg.sender)>=_dm, "Space Station: Misscalculation on Dark Matter token.");
        DarkMatter.burn(msg.sender, _dm);
        xpGained[1] = _dm/2;
    }

    if(_O2tanks > 0 && data.feature[2] < 256 ){
        require(inventory.balanceOf(msg.sender, O2tanksID) >= _O2tanks, "Space Station: Misscalculation on O2 tanks.");
    ...
    if(_numDMNFT >0 && data.feature[3] < 256){
        require(inventory.balanceOf(msg.sender, DMID) >= _numDMNFT, "Space Station: Misscalculation on Dark Matter NFTs.");
    ...
    for(uint256 index= 0; index < 3; index++){
        ...
        while(xpGained[index] > LVLS[data.levels[index]] && data.levels[index] < 100){
    ...
    if(adjustNFT){
        inventory.changeFeaturesForItem(_tokenID, uint8(data.feature[0]), uint8(data.feature[1]), uint8(data.feature[2]), uint8(data.feature[3]), stats.equipmentPosition, msg.sender);
        data.rate = (data.feature[0] + data.feature[1] + 1) * (50 + data.feature[3]);
    }
...
function trainAstronautXP(uint256[3] memory xpGained, uint256 _tokenID) nonReentrant{
...
function resupply(uint256 _tokenID, uint256 _O2tanks) nonReentrant{
    ...
    require(data.registered, "Space Station: Astronaut not registered");
    require(data.collectionStart > block.timestamp, "Space Station: Astronaut in Orbit");
    require(inventory.balanceOf(msg.sender, O2tanksID) >= _O2tanks,"Space Station: Misscalculation on supplies");
    
    uint256 timeDifference = (data.collectionStart - block.timestamp)/ (2 * _O2Tanks);
    inventory.burn(msg.sender, O2tanksID, _O2tanks);
